...
# python gram_accuracy.py output2.seqs output.3digitICD9.seqs output2 output3
...
def calculate_cost(test_model, dataset, options):
    ...
    for index in xrange(n_batches):
        ...
        for visit_idx in range(len(y)):
            for patient_idx in range(len(y[0])):
                top_k = idx_sorted[visit_idx][patient_idx][:20] # top20
    ...
    top_20_accuracy = truePositive / (truePositive + trueNegative)

    return costSum / dataCount, top_20_accuracy
...
def train_GRAM(
    ...
    get_cost = theano.function(inputs=[x, y, mask, lengths], outputs=[cost_noreg, y_hat], name='get_cost')
    print 'done!!'
    ...
    for epoch in xrange(max_epochs):
        ...
        validCost, validAccuracy = calculate_cost(get_cost, validSet, options)
        testCost, testAccuracy = calculate_cost(get_cost, testSet, options)
        buf = 'Epoch:%d, Duration:%f, Train_Cost:%f, Valid_Cost:%f, Test_Cost:%f, Valid_Accuracy:%f, Test_Accuracy:%f' % (epoch, duration, trainCost, validCost, testCost, validAccuracy, testAccuracy)
        print buf
    ...
    buf = 'Best Epoch:%d, Avg_Duration:%f, Train_Cost:%f, Valid_Cost:%f, Test_Cost:%f, Valid_Accuracy:%f, Test_Accuracy:%f' % (bestEpoch, epochDuration/max_epochs, bestTrainCost, bestValidCost, bestTestCost, bestValidAccuracy, bestTestAccuracy)
    print buf
    print2file(buf, logFile)
